...
/*
        public void SetAllChannels(ushort[] channels) {
            if (channels.Length != 12) throw new Exception("Please supply exactly 12 targetvalues");
            try
            {
              
                    MonoUsbDeviceHandle handle = new MonoLibUsb.MonoUsbDeviceHandle(deviceHandle);
                    byte[] cmdpayload = new byte[27];
                cmdpayload[0] = 0x9F;
                cmdpayload[1] = 0x0C;
                cmdpayload[2] = 0x00;
                    int i = 3;    
                foreach (ushort chan in channels) {
                    cmdpayload[i] =  (byte)(chan & 0x7F);
                    cmdpayload[i + 1] = (byte)((chan >> 7) & 0x7F);
                        i = i + 2;
                    }
                 
      int transferred = 0;

       MonoUsbApi.BulkTransfer(handle, 0x2, cmdpayload, cmdpayload.Length, out transferred, (ushort)5000);
