...
// Changed BY WIPO on JAN 2012
//          -> info: http://wipo-quadrocopter.blogspot.com/
//          -> ATTENTION read the how to on my blog before using this firmware
...
{ 
  ...
  delay(2000);
  if (waitforcommand()) //check if there is charecter in the serial buffer
  {
    AN_OFFSET[0] = readInt();
    AN_OFFSET[1] = readInt();
    AN_OFFSET[2] = readInt();
...
void loop() //Main Loop
{
  if((micros()-timer)>=9999)  // Main loop runs at 100hz runs always when 1 ms has passed
  {
    counter++;
    timer_old = timer;
    timer=micros();
    ...
    if (counter > 2)  // Read compass data and accel data at 50Hz... (2 loop runs)
    {
