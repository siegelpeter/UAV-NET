...
// recieves x,y and z accelerometer registers
void Read_Accel()
{
  int i = 0;
  byte buff[6];
  ...
  Wire.beginTransmission(AccelAddress); 
  Wire.write(0x32);        //sends address to recieve from
...
void Compass_Init()
{
  ...
  Wire.write(byte(0x00));   // set compass data rate
  Wire.write(0x18);   // to 50hz
  Wire.endTransmission(); //end transmission
  
  Wire.beginTransmission(CompassAddress);
  Wire.write(0x02); 
  Wire.write(byte(0x00));   // Set continouos mode
  Wire.endTransmission(); //end transmission
}
...
void Read_Compass()
{
  ...
  Wire.beginTransmission(CompassAddress); 
  Wire.write(0x03);        //sends address to recieve from
  Wire.endTransmission(); //end transmission
